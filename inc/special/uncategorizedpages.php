...
$revisions = swQuery(array('SELECT _name',
'SELECT _name WHERE _category',
'EXCEPT _name'));
...
if ($start>0)
	$navigation .= '<a href="index.php?name=special:uncategorized-pages&start='.sprintf("%0d",$start-$limit).'"> '.swSystemMessage('back',$lang).'</a> ';
		
$navigation .= " ".sprintf("%0d",min($start+1,$count))." - ".sprintf("%0d",min($start+$limit,$count))." / ".$count;
if ($start<$count-$limit)
	$navigation .= ' <a href="index.php?name=special:uncategorized-pages&start='.sprintf("%0d",$start+$limit).'">'.swSystemMessage('forward',$lang).'</a>';
