...
;
		...
		for($i = 1; $i < count($list); $i++)
		{
			...
			if ($vp === FALSE) continue;
			
			$v = substr($list[$i],0,$vp);
			...
			$list[$i] = $link.substr($list[$i],$vp+2);	
		...
		for($i = 1; $i < count($list); $i++)
		{
			...
			if (!$label) $label = $val;
			$r = '';
			$r = swGetCurrentRevisionFromName('Image:'.$val);
			if ($r)
