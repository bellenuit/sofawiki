...
$revisions = swQuery(array('SELECT _name FROM image:','CALC n _name URLIFY %(.*)/.*% $1 REGEX',
'PROJECT n','SELECT _link','CALC n _link URLIFY %(.*)/.*% $1 REGEX media: image: REPLACE','PROJECT n','EXCEPT n'));
...
if ($start>0)
	$navigation .= '<a href="index.php?name=special:unused-files&start='.sprintf("%0d",$start-$limit).'"> '.swSystemMessage('back',$lang).'</a> ';
		
$navigation .= " ".sprintf("%0d",min($start+1,$count))." - ".sprintf("%0d",min($start+$limit,$count))." / ".$count;
if ($start<$count-$limit)
	$navigation .= ' <a href="index.php?name=special:unused-files&start='.sprintf("%0d",$start+$limit).'">'.swSystemMessage('forward',$lang).'</a>';
